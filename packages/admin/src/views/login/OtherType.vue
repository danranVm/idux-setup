...
const navNextShow = computed(() => {
    return (navListRef.value?.offsetWidth ?? 0) - navOffset.value > navWrapperRef.value?.offsetWidth ?? 0
})
...
<template>
    <div class="other-login-wrapper">
        ...
        <IxIcon class="other-login-nav-pre" name="left" v-show="navPreShow" @click="navPreClick"></IxIcon>
        <IxIcon class="other-login-nav-next" name="right" v-show="navNextShow" @click="navNextClick"></IxIcon>
        <div class="other-login-nav-wrapper" ref="navWrapperRef">
            <div class="other-login-nav-list" :style="navListStyle" ref="navListRef">
                <div class="other-login-nav text-center" :style="{ 'width': navWidth + 'px' }"
                    v-for="(item, index) in dataSource" :key="index">
                    <IxTooltip :title="item.title" placement="top">
                        <IxButton :icon="item.icon" shape="circle" @click="item.onClick">
                        </IxButton>
                    </IxTooltip>
                </div>
            </div>
